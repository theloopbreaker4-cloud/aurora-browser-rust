...
impl AuroraDelegate {
    ...
    fn handle_simple_dialog(&self, dialog: ServoSimpleDialog) {
        use crate::platform::dialogs::{simple_dialog, SimpleDialogKind, SimpleDialogResult};
        ...
        match status {
            ...
            LoadStatus::Complete => {
                ...
                if let Some(url) = webview.url() {
                    ...
                    crate::pages::history::push_history_entry(&title, &url_s);
                }
            }
            _ => {}
        }
        ...
        match control {
            EmbedderControl::FilePicker(mut picker) => {
                #[cfg(windows)]
                {
                    ...
                    let filters = if exts.is_empty() {
                        ...
                        vec![crate::platform::dialogs::FileFilter {
                            description: format!("Allowed ({})", exts.join(", ")),
                            extensions: exts,
                        }]
                    };
                    let multi = picker.allow_select_multiple();
                    match crate::platform::dialogs::open_file_dialog(self.state.parent_hwnd, multi, &filters)
                    {
                        Some(paths) => {
                            picker.select(&paths);
                            picker.submit();
            ...
            EmbedderControl::SelectElement(mut select) => {
                #[cfg(windows)]
                {
                    ...
                    let items: Vec<crate::platform::dialogs::PopupItem> = select
                        .options()
                        .iter()
                        .map(|opt_or_grp| match opt_or_grp {
                            SelectElementOptionOrOptgroup::Option(o) => {
                                crate::platform::dialogs::PopupItem::Item {
                                    id: o.id as u32,
                                    label: o.label.clone(),
                            ...
                            SelectElementOptionOrOptgroup::Optgroup { label, options } => {
                                crate::platform::dialogs::PopupItem::Group {
                                    label: label.clone(),
                                    items: options
                                        .iter()
                                        .map(|o| crate::platform::dialogs::PopupItem::Item {
                                            id: o.id as u32,
                                            label: o.label.clone(),
                                            checked: selected.contains(&o.id),
                                            disabled: o.is_disabled,
                    ...
                    match crate::platform::dialogs::popup_menu(self.state.parent_hwnd, sx, sy, &items) {
                        Some(id) => {
            ...
            EmbedderControl::ColorPicker(mut picker) => {
                #[cfg(windows)]
                {
                    ...
                    let chosen = crate::platform::dialogs::pick_color(self.state.parent_hwnd, initial)
            ...
            EmbedderControl::ContextMenu(menu) => {
                #[cfg(windows)]
                {
                    ...
                    let items: Vec<crate::platform::dialogs::PopupItem> = menu
                        .items()
                        .iter()
                        .map(|it| match it {
                            ...
                            ServoContextMenuItem::Separator => crate::platform::dialogs::PopupItem::Separator,
                        })
                        .collect();
