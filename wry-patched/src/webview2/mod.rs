  ...
  unsafe fn attach_ipc_handler(
    ...
    webview.add_WebMessageReceived(
      &WebMessageReceivedEventHandler::create(Box::new(move |_, args| {
        ...
        // Guard against empty URI (can occur in child webview mode on first message)
        let uri: String = if url.is_empty() { "wry://ipc".to_string() } else { url };
        if let Ok(req) = Request::builder().uri(uri.as_str()).body(js) {
          ipc_handler(req);
        }
